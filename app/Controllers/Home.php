    ...
    if(! empty($users))
    {
      if($users['status_akun'] == "1" && $users['status'] == "verified")
      {
        if($users && password_verify($this->request->getVar('password_hash'), $users['password_hash']))
        {
          ...
          else if($check_session['role_id'] == 5) 
          {
            session()->setFlashdata('message', 'Welcome, ' . $check_session['username'] . '.');

            return redirect()->route('manager_dashboard');
          }
          else if($check_session['role_id'] == 6) 
          {
            session()->setFlashdata('message', 'Welcome, ' . $check_session['username'] . '.');

            return redirect()->route('finance_dashboard');
          }
          else if($check_session['role_id'] == 7) 
          {
            session()->setFlashdata('message', 'Welcome, ' . $check_session['username'] . '.');

            return redirect()->route('officer_dashboard');
          }
        } 
