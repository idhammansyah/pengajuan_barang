...
class Manager_Controller extends BaseController
{
  protected $views;
  ...
  {
    $input = [
      'status_approval' => "Accepted",
      'approval_manager_by'  => $this->session->get('user_data')['username'],
      'approval_manager_at'  => date('Y-m-d H:i:s')
    ];

    if($this->purchasing->update($id, $input)) {
      $response = "Approved Success!";
      return redirect()->route('view_purchasingss')->with('message', $response);
    } else {
      $response = "Failed Approved Data!";
      return redirect()->route('view_purchasingss')->with('error', $response);
    }
  }
  ...
  public function submit_reason_manager($id)
  {
    $input =[
      'status_approval' => "Rejected",
      'reason_rejected_manager' => $this->request->getVar('alasan'),
      'approval_manager_by'  => $this->session->get('user_data')['username'],
      'approval_manager_at'  => date('Y-m-d H:i:s')
    ];
    if($this->purchasing->update($id, $input)) {
